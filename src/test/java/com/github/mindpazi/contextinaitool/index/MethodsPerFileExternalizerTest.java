...
public class MethodsPerFileExternalizerTest {
    ...
    public void testSaveAndRead() throws IOException {
        ...
        List<MethodMeta> originalMethods = Arrays.asList(
            new MethodMeta("com.example.TestClass", "method1", "/path/to/file.java", 10, 15),
            new MethodMeta("com.example.TestClass", "method2", "/path/to/file.java", 20, 25),
            new MethodMeta("com.example.OtherClass", "method3", "/path/to/other.java", 5, 12)
        );
        ...
        for (int i = 0; i < originalMethods.size(); i++) {
            ...
            assertEquals(original.startLine(), read.startLine());
            assertEquals(original.endLine(), read.endLine());
        }
    }
